...
ggplot(MAbirds, aes(x=observations_per_min)) + 
  geom_density() #I cannot see the normal distribution
ggplot(MAbirds, aes(x=log10(observations_per_min))) + 
  geom_density() #The log10 transformed data looks decent

#Save output to csv
write.csv(MAbirds, row.names = FALSE, './Data/Processed/Massachusetts/MAbirds.csv')
```

## Summarize Bird Data

```{r summarize.bird.data}
...
write.csv(OSPR_MA_temp, row.names = FALSE, './Data/Processed/Massachusetts/OSPR_MA_temp.csv')
write.csv(RWBL_MA_temp, row.names = FALSE, './Data/Processed/Massachusetts/RWBL_MA_temp.csv')
write.csv(WODU_MA_temp, row.names = FALSE, './Data/Processed/Massachusetts/WODU_MA_temp.csv')
```

## Correlation Plots
> What does the relationship between bird observations, time, and temperature look like?
...
#Strongest relationship between month and the observational variables of the three species, which makes sense because the Osprey is the species that leaves Massachusetts entirely of the three observed
...
#Least exciting of the three plots
```
> Observations: Based on the correlation plots, the strongest, non-colinear relationships appear to be between observations per minute and average monthly temperature, observations per minute and month, and observations per minute and season. Across the three species, there also appear to be associations between temperature/time and observation count/presence. However, the statistical analysis for MA will focus on effort-adjusted observation data only.
