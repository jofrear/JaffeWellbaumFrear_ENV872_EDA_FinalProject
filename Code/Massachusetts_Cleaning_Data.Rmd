...
OSPR_MA_month <- OSPR_MA_ebd %>% 
  ...
  mutate(Common_Name = "Osprey",
         State = "Massachusetts") %>% 
  select(Common_Name, State, Observations, Presence, Month_Year, AvgMonthlyTemp_F)
...
#OSPR_MA_week <- OSPR_MA_ebd %>% 
  ...
  #summarize(Weekly_Obs = sum(observation_count),
            #Weekly_Pres = sum(Presence))

#Create a summary dataframe of bird observations by merging month/week
#OSPR_MA_obs <- merge(x=OSPR_MA_month, y=OSPR_MA_week, by='Month_Year', all.x=TRUE) %>% 
...
RWBL_MA_month <- RWBL_MA_ebd %>% 
  group_by(Observations = sum(observation_count),
           Presence = sum(Presence),
            AvgMonthlyTemp_F = first(AvgMonthlyTemp_F)) %>%
...
## Plot results
```{r ospr.presence}
# Presence plot for entire date range
OSPRpresplot <- ggplot(OSPR_MA_month, aes(x=Month_Year, y=Presence, color=AvgMonthlyTemp_F)) +
  geom_line(lwd=0.8) +
  scale_color_viridis(option='plasma') +
  scale_x_date(date_breaks = '1 year', date_labels = '%Y') +
labs(x='Date', color='Mean Temperature (F)', title = 'Osprey', subtitle = 'Massachussetts') +
  theme(legend.position = 'bottom')

print(OSPRpresplot)

# Presence plot by year
OSPRpreplot2 <- ggplot(OSPR_MA_month, aes(x=month(Month_Year), y=Presence, group=year(Month_Year), color=AvgMonthlyTemp_F)) +
  geom_line(lwd=0.8) +
  scale_color_viridis(option='plasma') + 
...
```{r rwbl.presence}
# Presence plot for entire date range
RWBLpresplot <- ggplot(RWBL_MA_month, aes(x=Month_Year, y=Presence, color=AvgMonthlyTemp_F)) +
  geom_line(lwd=0.8) +
  scale_color_viridis(option='plasma') +
  scale_x_date(date_breaks = '1 year', date_labels = '%Y') +
labs(x='Date', color='Mean Temperature (F)', title = 'Red-Winged Blackbird', subtitle = 'Massachussetts') +
  theme(legend.position = 'bottom')
...
# Presence plot by year
RWBLpresplot2 <- ggplot(RWBL_MA_month, aes(x=month(Month_Year), y=Presence, group=year(Month_Year), color=AvgMonthlyTemp_F)) +
  geom_line(lwd=0.8) +
  scale_color_viridis(option='plasma') + 
  scale_x_continuous(breaks = c(1:12), labels = c('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')) +
labs(x='Date', color='Mean Temperature (F)', title = 'Red-Winged Blackbird', subtitle = 'Massachussetts') +
...
```{r wodu.presence}
# Presence plot for entire date range
WODUpresplot <- ggplot(WODU_MA_month, aes(x=Month_Year, y=Presence, color=AvgMonthlyTemp_F)) +
  geom_line(lwd=0.8) +
  scale_color_viridis(option='plasma') +
  scale_x_date(date_breaks = '1 year', date_labels = '%Y') +
labs(x='Date', color='Mean Temperature (F)', title = 'Wood Duck', subtitle = 'Massachussetts') +
  theme(legend.position = 'bottom')
...
# Presence plot by year
WODUpresplot2 <- ggplot(WODU_MA_month, aes(x=month(Month_Year), y=Presence, group=year(Month_Year), color=AvgMonthlyTemp_F)) +
  geom_line(lwd=0.8) +
  scale_color_viridis(option='plasma') + 
  scale_x_continuous(breaks = c(1:12), labels = c('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')) +
labs(x='Date', color='Mean Temperature (F)', title = 'Wood Duck', subtitle = 'Massachussetts') +
...
## Osprey failed first/last observation
```{r include=FALSE}
